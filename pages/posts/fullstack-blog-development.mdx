...
architecture-beta
    ...
    group deploy(cloud)[Deployment Layer]

    service github(internet)[GitHub Repository] in api
    service vercel(internet)[Vercel Platform] in api
    service cdn(internet)[Global CDN] in api

    service nextjs(server)[Next.js App] in app
    service nextra(server)[Nextra Engine] in app
    service components(server)[React Components] in app

    service mdx(database)[MDX Files] in data
    service assets(database)[Static Assets] in data
    service config(database)[Configuration] in data

    service actions(cloud)[GitHub Actions] in deploy
    service build(cloud)[Build Process] in deploy
    service edge(cloud)[Edge Functions] in deploy
